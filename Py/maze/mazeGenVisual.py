...
def drawMaze():
    ...
    pyg.draw.circle(
        ...
        "red",
        (
            grid[-1][-1].cordTopLeft[0] + (blockSize / 2),
            grid[-1][-1].cordTopLeft[1] + (blockSize / 2),
        ),
        blockSize / 3,
    )
...
def getNebors(node):
    ...
    try:
        ...
        if (
            not grid[int(node.gridposY - 1)][int(node.gridposX)].viseted
            and grid[int(node.gridposY - 1)][int(node.gridposX)].gridposY
            == node.gridposY - 1
        ):
            valideNebors.append(grid[int(node.gridposY - 1)][int(node.gridposX)])
        if (
            not grid[int(node.gridposY)][int(node.gridposX + 1)].viseted
            and grid[int(node.gridposY)][int(node.gridposX + 1)].gridposX
            == node.gridposX + 1
        ):
...
def get_valPath(node):
    valid = []
    for neighbor in node.linkedNodes:
        ...
        for colom in range(0, WINDOW_HIGHT, blockSize):
            ...
            if grid[y][x].explored:
                pyg.draw.circle(
                    SCREEN,
                    "blue",
                    (colom + (blockSize / 2), row + (blockSize / 2)),
                    blockSize / 4,
                    blockSize / 4,
                )
